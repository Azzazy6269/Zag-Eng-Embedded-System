...
Std_ReturnType SPI_Send_String(const SPI_Config *config, const uint8_t *str) {
    Std_ReturnType ret;
    while (*str != '\0') {  // Continue until the null terminator is reached
        ret = SPI_Send_Byte(config, *str);  // Send each character
        str++;  // Move to the next character
        __delay_ms(10);
    }
    return SPI_Send_Byte(config, '\0');  // Send null terminator to mark end of string
}

Std_ReturnType SPI_Read_String(const SPI_Config *config, uint8_t *buffer, uint8_t max_length) {
    Std_ReturnType ret;
    uint8_t data;
    uint8_t index = 0;

    while (index < max_length - 1) {  // Leave room for null terminator
        ret = SPI_Read_Byte(config, &data);  // Read each byte  
        if (data == '\0') {  // Check for end of string
            break;
        }
        buffer[index++] = data;  // Store the received character
    }

    buffer[index] = '\0';  // Null-terminate the received string
    return E_OK;
}
